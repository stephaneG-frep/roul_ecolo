...
//}
    ...
    if (isset($_GET['depart'])) {
        $searchTerm = trim($_GET['depart']);
        if (!empty($searchTerm)) {
            $annonces = $annonceModel->getAnnonceByDepart($searchTerm);
        }
    }
    if(isset($_SESSION['id'])){
    ?>
    ...
    <h1>Rechercher des annonces Ecoride</h1>
        <form method="GET" class="search-form">
            <input type="text" name="depart" placeholder="Entrez une ville de depart.)" 
            // Traitement de la recherche
            if (isset($_GET["depart"])) {
                $searchTerm = trim($_GET["depart"]);
                if (!empty($searchTerm)) {
                    $annonces = $annonceModel->searchByDepart($searchTerm);
                }
            }
        value="<?= htmlspecialchars($searchTerm) ?>">
            <button type="submit">Rechercher</button>
        ...
        <?php if (!empty($searchTerm)): ?>
                    <div class="search-info">
                        <?php if (!empty($annonces)): ?>
                            <p>Résultats pour le départ : "<?= htmlspecialchars($searchTerm) ?>"</p>
                        <?php else: ?>
                            <p>Aucun résultat trouvé pour le départ : "<?= htmlspecialchars($searchTerm) ?>"</p>
                        <?php endif; ?>
                    </div>
                <?php endif; ?>
                ...
                <?php elseif (empty($searchTerm)): ?>
                    <div class="no-results">
                        <p>Entrez unne ville  de départ pour afficher les annonces correspondantes</p>
                    </div>
                <?php endif; ?>
        </div>
